    ...
    def create_distributions(self):
        ...
        for i, j in enumerate(self.my_data.stoch_data['types']):
            if j == 'Uniform':
                ...
                mean = self.my_data.stoch_data['mean'][i]
                std = self.my_data.stoch_data['deviation'][i]
                a = (mean/std)**2.
                b = a/mean
                print(a,b)
                self.dists[i] = sp.stats.gamma(
                    a, scale = b)
                self.polydists[i] = sp.stats.gamma(1.)
                self.polytypes[i] = 'Laguerre'
                ...
                b = a * (1./mean - 1.)
                print(a,b)
                self.dists[i] = sp.stats.beta(a, b)
                self.polydists[i] = sp.stats.beta(a, b)
                self.polytypes[i] = 'Jacobi'
            else:
                raise ValueError(""" Distribution for %s not found.
                                     Choose between Uniform, Gaussian,
                                     Gamma and Beta."""
    ...
    def uq_calc_A(self, multindices):
        ...
        for i, multiindex in enumerate(multindices):
            for j in range(dimension):
                ...
                elif self.my_experiment.polytypes[j] == 'Laguerre':
                    '''
                    Note: We use the GENERALIZED Laguerre polynomial
                    '''
                    mean = self.my_experiment.my_data.stoch_data['mean'][j]
                    std = self.my_experiment.my_data.stoch_data['deviation'][j]
                    alpha = (mean/std)**2.
                    beta = alpha/mean
                    A[:,i] *= sp.special.eval_genlaguerre(deg,alpha,U[:,j]/beta)
    ...
    def get_psi_sq(self):
        ...
        for i in range(n_terms):
            for j in range(d):
                ...
                if self.my_experiment.polytypes[j] == 'Legendre':
                ...
                elif self.myExperiment.polytypes[j] == 'Laguerre':
                    '''
                    Note: [0,inf]
                    '''
                    mean = self.my_experiment.my_data.stoch_data['mean'][j]
                    ...
                    a = (mean/std)**2.
                    b = alpha/mean
                    xi, wi = sp.special.la_roots(deg+1,a-1)
                    poly = sp.special.genlaguerre(deg,a)**2
                    
                    psiSq[i] *= 1.0/(sp.special.gamma(a)) * sum(wi*poly(xi/b))
        ...
        for count, elem in enumerate(permutations):
            ...
            for l in range(dimension):
                ...
                if self.my_experiment.polytypes[l] == 'Legendre':
                    ...
                    mean = self.my_experiment.my_data.stoch_data['mean'][l]
                    std = self.my_experiment.my_data.stoch_data['deviation'][l]
                    a = (mean/std)**2.
                    b = alpha/mean
    ...
    def get_psi_four(self):
        ...
        for count, elem in enumerate(permutations):
            ...
            for m in range(dimension):
                ...
                if self.my_experiment.polytypes[m] == 'Legendre':
                ...
                elif self.myExperiment.polytypes[m] == 'Laguerre':
                    mean = self.my_experiment.my_data.stoch_data['mean'][m]
                    std = self.my_experiment.my_data.stoch_data['deviation'][m]
                    a = (mean/std)**2.
                    b = alpha/mean
                    ...
                    poly = sp.special.genlaguerre(degi,a) * \
                           sp.special.genlaguerre(degj,a) * \
                           sp.special.genlaguerre(degk,a) * \
                           sp.special.genlaguerre(degl,a)
                           
                    psiFour[count] *= 1.0/(sp.special.gamma(a)) * sum(wi*poly(xi/b))
