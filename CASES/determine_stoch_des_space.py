# -*- coding: utf-8 -*-
"""
Created on Mon Mar 04 23:49:41 2019

@author::  tsiri
@subject:: Test case description: heat exchanger
...
def check_dictionary(run_dict, uq = False):
    ...
    if not uq:
        ...
        if not list(
            ...
            raise ValueError(
                'Please select "DET" or "ROB" as a key for the "objectives" value.')
        elif not isinstance(list(run_dict['objectives'].values())[0], tuple):
            raise TypeError(
                'The value of the key "DET" or "ROB" should be a tuple with the weights for the objectives (1 for maximization, -1 for minimization).')
        elif not all(isinstance(weight, int) for weight in list(run_dict['objectives'].values())[0]):
            raise TypeError('The weights should be equal to 1 or -1.')
        elif not all(abs(weight) == 1 for weight in list(run_dict['objectives'].values())[0]):
            raise ValueError('The weights should be equal to 1 or -1.')

        if not isinstance(run_dict['stop'], tuple):
            raise TypeError(
                'The value of the key "stop" should be a tuple with two elements.')

        if not run_dict['stop'][0] == 'BUDGET':
            raise ValueError(
                'The first element in the tuple related to the key "stop" should be equal to "BUDGET".')

        if not isinstance(run_dict['stop'][1], int):
            raise TypeError(
                'The second element in the tuple related to the key "stop" should be a positive integer.')

        if not isinstance(run_dict['population size'], int):
            raise TypeError(
                'The value of the key "population number" should be a positive integer.')

        if not isinstance(run_dict['x0'], tuple):
            raise TypeError(
                'The value of the key "x0" should be a tuple with two elements.')

        if not run_dict['x0'][0] == 'AUTO' and not run_dict['x0'][0] == 'CUSTOM':
            raise ValueError(
                'The first element in the tuple related to the key "x0" should be equal to "AUTO" or "CUSTOM".')
        elif run_dict['x0'][0] == 'AUTO' and not run_dict['x0'][1] == 'RANDOM' and not run_dict['x0'][1] == 'LHS':
            raise ValueError(
                'When selecting "AUTO", the second element in the tuple related to the key "x0" should be equal to "RANDOM" or "LHS".')
        ...
        if not isinstance(run_dict['cx prob'], float):
            raise TypeError(
                'The value of the key "cx prob" should be a positive float, lower or equal to 1.')
        ...
        if not isinstance(run_dict['mut prob'], float):
            raise TypeError(
                'The value of the key "mut prob" should be a positive float, lower or equal to 1.')
        ...
        if not isinstance(run_dict['eta'], float):
            raise TypeError(
                'The value of the key "eta" should be a positive float, lower or equal to 1.')
        elif run_dict['eta'] > 1.:
            raise ValueError(
                'The value of the key "eta" should be a positive float, lower or equal to 1.')
        ...
        if not isinstance(run_dict['print results light'][0], bool):
            raise TypeError(
                'The value of the key "print results light" should be a True or False.')

        if run_dict['print results light'][0] and not len(
                run_dict['print results light']) == 2:
            raise ValueError(
                'When light result printing is desired, provide the step for printing a generation in the form of a positive integer as the second list item')
        ...
        if run_dict['draw pdf cdf'][0] and not len(run_dict['draw pdf cdf']) == 2:
            raise ValueError(
    ...
    if rob or uq:    
        ...
        if not isinstance(run_dict['pol order'], int):
            raise TypeError(
                'The value of the key "pol order" should be a positive integer.')
    ...
    if rob:
        ...
        if not len(list(run_dict['objectives'].values())[0]) == 2*len(run_dict['objective of interest']):
                    raise ValueError(
                        """The number of objectives of interest should be equal to
                           two times the number of weigths. Note that the weigths
                           with uneven indices relate to the standard deviation of
                           the quantity of interest and should therefore be 
                           equal to -1 (minimization).""")
...
class stochastic_design_space(object):
    ...
    def __init__(self,opt_type,case,design_space):
        """
        Class which creates an object which characterizes the stochastic design space
        for the system model evaluation.
    ...
    def read_design_space(self):
        ...
        ## --------------------- READ 'design_space' FILE ----------------------
        ##---------------------------------------------------------------------

        path_to_read = os.path.join(self.case_path,self.design_space)
        
        if not os.path.isfile(path_to_read):
            raise ValueError('Missing file: "design_space" or the name of the case does not exist. Make sure that the name of the case is equal to the name of the folder in CASES.')
        ...
        with open(path_to_read, 'r') as f:
            for l in f:
                ...
                if tmp[1] == 'par':
                    if len(tmp) != 3:
                        raise IndexError(""" Wrong characterization of the parameter %s. 
                                            Is it supposed to be a variable? Change "par" into "var".""" %tmp[0])

                    self.par_dict[tmp[0]] = float(tmp[2])
                    
                elif tmp[1] == 'var':
                    if len(tmp) != 4:
                        raise IndexError(""" Wrong characterization of the design variable %s. 
                                            Is it supposed to be a parameter? Change "var" into "par".""" %tmp[0])
                    ...
                    if float(tmp[2]) >= float(tmp[3]):
                        raise NameError('The lower bound is equal or greater than the upper bound of %s' %tmp[0])
                else:
                    raise ValueError(""" The line %s does not mention if the characterization 
                                        corresponds to a parameter ("par") or a design variable ("var").""" %tmp)
        self.n_dim = len(self.var_dict.keys())
        self.nPar = len(self.par_dict.keys())
        ...
        if not self.var_dict and 'UQ' not in self.opt_type:     
            raise NameError('Variable list is empty! Case cannot be instatiated!')
                    
    def read_stochastic_space(self):
        ## ------------------ READ UNCERTAINTIES ('stochastic_space' file) -------------------
        ##------------------------------------------------------------------------------------

        path_to_read = os.path.join(self.case_path,'stochastic_space')

        if any(x in self.opt_type for x in ['ROB','UQ']) and os.path.isfile(path_to_read):
            ...
            raise NameError('Missing file: "stochastic_space". Uncertainty matrix cannot be built!')
