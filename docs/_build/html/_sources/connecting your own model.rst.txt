...
To illustrate, importing data from 3 data files (e.g. :file:`datafile_1.csv`, :file:`datafile_2.csv`, :file:`datafile_3.csv`) can be performed as follows::
...
The model evaluation is done in the function :py:func:`evaluate()`. This function is called
...
The dictionary `x[1]` in the enumerate object :py:obj:`x` consists of the model parameter names and design variable names, as defined in :file:`design_space` and :file:`stochastic_space`, as keys.
The corresponding values are the values generated by the optimization or uncertainty quantification algorithm.

To illustrate, when 4 model parameters and 3 design variables are present, the dictionary can be constructed as follows::
...
In addition to the enumerate object :py:obj:`x`, the :py:func:`evaluate()` function takes `params` as an optional argument.
This list contains the imported data, returned by the function :py:func:`set_params()`. When :py:func:`set_params()` does not exist,
i.e. when no data needs to be imported before the model evaluations commence, `params` corresponds to an empty list.
...
The dictionary with the input sample names and values `x[1]` is passed directly as an argument to the :py:func:`four_bar_truss` function.  
...
The :py:func:`run_energyplan()` function is evaluated in :py:func:`evaluate()`, where the enumerate object `x` is provided as an argument. For this case, no fixed parameters are provided as an argument::
