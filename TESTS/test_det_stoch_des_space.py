...
def define_case():
    ...
    if not os.path.exists(path_case):
        ...
        shutil.copy(os.path.join(path,'CASES','NO_MODEL','case_description.py'),path_case)
...
def test_load_case_uq(define_case):
    run_dict = {'case': 'CASE1'}
    test_obj, test_case_obj = load_case(run_dict, 'design_space',uq = True)
    assert test_obj.case == 'CASE1'
    #assert test_obj.lb = []
    #assert test_obj.ub = []
...
def test_load_case_det(define_case):
    ...
    load_case(run_dict, 'design_space')
